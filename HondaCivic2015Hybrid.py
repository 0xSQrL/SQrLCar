...
class HondaCivic:
    ...
    def get_speed_mph(self):
        if HondaCivic.DEBUG:
            return 10.5
        return math.floor(self.connection.query(obd.commands.SPEED).value.to("mph").magnitude * 5) / 5

    def get_odometer(self):
        if HondaCivic.DEBUG:
            return 12
        return self.connection.query(HondaCivic.odomCMD, force=True).value.magnitude

    def get_running_time(self):
        if HondaCivic.DEBUG:
            return 400
        return self.connection.query(obd.commands.RUN_TIME).value.to("mph").magnitude

    def get_battery(self):
        if HondaCivic.DEBUG:
            return 0.69
        return self.connection.query(obd.commands.HYBRID_BATTERY_REMAINING).value.magnitude

    def get_pressure(self):
        if HondaCivic.DEBUG:
            return 98
        return self.connection.query(obd.commands.BAROMETRIC_PRESSURE).value.magnitude

    def get_gas_percent(self):
        if HondaCivic.DEBUG:
            return 0.72
        return self.connection.query(obd.commands.FUEL_LEVEL).value.magnitude
